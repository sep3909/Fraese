...
uint16_t adc_values[4];             //$ alle vier Tempsensoren werden in dem Array gespeichert bzw. deren Werte
...
void temp_messung(void){
    ...
    float temp = 1.0f / ((1.0f/T_N) + (1.0f/B) * log(R_NTC/R_N));

    if (temp >= 350 && Lüfter_an == false){
        Lüfter_an = true;
